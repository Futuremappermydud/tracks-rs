...
pub fn ease_in_expo(p: f32) -> f32 {
  if p == 0.0 { p } else { 2.0f32.powf(10.0 * (p - 1.0)) }
}

pub fn ease_out_expo(p: f32) -> f32 {
  if p == 1.0 { p } else { 1.0 - 2.0f32.powf(-10.0 * p) }
}
...
pub fn ease_in_out_elastic(p: f32) -> f32 {
  if p < 0.5 {
      0.5 * (13.0 * std::f32::consts::FRAC_PI_2 * (2.0 * p)).sin() * 2.0f32.powf(10.0 * ((2.0 * p) - 1.0))
  } else {
      0.5 * (((-13.0 * std::f32::consts::FRAC_PI_2 * (2.0 * p)).sin() * 2.0f32.powf(-10.0 * ((2.0 * p) - 1.0))) + 2.0)
  }
}
