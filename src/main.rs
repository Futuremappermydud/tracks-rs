use std::{borrow::{Borrow, BorrowMut}, error::Error, time::SystemTime};

use easings::{functions::Functions};
use minifb::{Key, Window, WindowOptions};
use plotters::{backend::BGRXPixel, chart::ChartBuilder, prelude::*, series::LineSeries, style::{Color, IntoFont, BLACK, GREEN, TRANSPARENT}};
use serde_json::json;
...
fn main() -> Result<(), Box<dyn Error>> {
  ...
  let value = Functions::EaseStep.interpolate(0.5);
            ...
            .into_drawing_area();
            {
                ...
                chart.draw_series(LineSeries::new(
                  (0f64..1f64)
                    .step(0.01)
                    .values()
                    .map(|x| (x, Functions::EaseInExpo.interpolate(x as f32) as f64)),
                    &BLUE,
                ))?;
            }
            root.present()?;
